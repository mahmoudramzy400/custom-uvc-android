...
void UVCPreview::callbackPixelFormatChanged() {
    ...
    switch (mPixelFormat) {
        case PIXEL_FORMAT_RAW:
            ...
            // If we're streaming MJPEG, allow passthrough
            if (frameFormatType == UVC_VS_FRAME_MJPEG) {
                LOGI("MJPEG passthrough enabled for RAW mode!");
                callbackPixelBytes = 0;  // Not used for passthrough
                mFrameCallbackFunc = NULL;
            } else {
...
//    time_t c_start, c_end;
    ...
    if (LIKELY(!result)) {
        ...
        LOGI("Streaming...");
#endif
        if (frameFormatType == UVC_VS_FRAME_MJPEG && mPixelFormat == PIXEL_FORMAT_RAW) {
            // MJPEG passthrough: send raw MJPEG frame to capture/callback!
            for (; LIKELY(isRunning());) {
                frame_mjpeg = waitPreviewFrame();
                if (LIKELY(frame_mjpeg)) {
                    // Directly add the raw MJPEG frame (no conversion!)
                    addCaptureFrame(frame_mjpeg);  // <- Pass as is
                }
            }
        } else if (frameFormatType == UVC_VS_FRAME_MJPEG) {
            // MJPEG mode
            for (; LIKELY(isRunning());) {
                frame_mjpeg = waitPreviewFrame();
                if (LIKELY(frame_mjpeg)) {
...
//    ENTER();
    ...
    if (LIKELY(frame)) {
        ...
        if (mFrameCallbackObj && iframecallback_fields.onFrame) {
            ...
            } else if (mPixelFormat == PIXEL_FORMAT_RAW && frameFormatType == UVC_VS_FRAME_MJPEG) {
                // Passthrough: send raw MJPEG bytes without any change
                callbackPixelBytes = frame->data_bytes;
                callback_frame = frame;
                //LOGI("MJPEG passthrough: sending %zu bytes", callbackPixelBytes);
